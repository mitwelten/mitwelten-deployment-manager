...
import { Deployment, Tag } from 'src/app/shared';
  ...
  connect(): Observable<Deployment[]> {
    ...
    if (this.paginator && this.sort) {
      ...
      this.tagOptions = data
          .map(d => d.tags ? d.tags : [])
          .flat()
          .reduce((unique_tags: Tag[], current_tag) => {
            return unique_tags.filter((t: Tag) => t.tag_id === current_tag.tag_id).length ? unique_tags : [...unique_tags, current_tag]
          }, [])
          .sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
