...
export class EnvironmentDataSource extends DataSource<Environment> {
  ...
  private getFilteredData(data: Environment[]): Environment[] {
    ...
    if (this.filter.controls['timestamp_s'].value !== null) {
      filteredData = filteredData.filter(e => (new Date(e.timestamp)) >= this.filter.controls['timestamp_s'].value);
    }

    if (this.filter.controls['timestamp_e'].value !== null) {
      filteredData = filteredData.filter(e => (new Date(e.timestamp)) <= this.filter.controls['timestamp_e'].value);
    }

    attrKeys.forEach(k => {
      if (this.filter.controls[k+'_s'].value !== null)
      filteredData = filteredData.filter(e => {
        const value = e[<keyof Environment>k];
        return value ? (value >= this.filter.controls[k+'_s'].value) : false;
      });
      if (this.filter.controls[k+'_e'].value !== null)
      filteredData = filteredData.filter(e => {
        const value = e[<keyof Environment>k];
        return value ? (value <= this.filter.controls[k+'_e'].value) : false;
      });
    })
